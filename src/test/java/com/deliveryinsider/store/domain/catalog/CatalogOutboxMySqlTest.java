...
class CatalogOutboxMySqlTest {
    ...
    @Test void newStoreConsumesExistingVerificationAndPublishesVersionOneAtomically() {
        ...
        var created = stores.create(2L, new com.deliveryinsider.store.domain.store.request.StoreCreateRequest(verification,
            "CATALOG NEW TEST STORE",null,"TEST ADDRESS",null,"TEST",1,0,"09:00","21:00"));
    ...
    @Test void storeUpdateAlsoAdvancesVersionAndEnqueuesSnapshot() {
        stores.update(1L, new StoreUpdateRequest("changed store", null, null, null, null, null, null, null, null, null));
        var event = json.readTree(jdbc.queryForObject("SELECT payload_json FROM outbox_events", String.class));
    ...
    private MenuCreateRequest create() { return new MenuCreateRequest("CATALOG TEST MENU",18000,5000,700,10,1); }
    private MenuUpdateRequest change(String name, MenuStatus status) { return new MenuUpdateRequest(name,null,null,null,null,null,status); }
}
